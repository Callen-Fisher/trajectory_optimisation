function [MinSoln,Problem,Guess,FVAL,LAMBDA,GRAD,HESSIAN,EXITFLAG,OUTPUT,adifuncs]=traj_opt_multipleshoot(nNodesF1,nNodesS,nNodesF2,pogo)
...
% %     end
    ...
    addpath('adigator');
    addpath('adigator/doc');
    addpath('adigator/examples');
    addpath('adigator/lib');
    addpath('adigator/util');
    addpath('adigator/lib/cadaUtils');
    ...
    setup.order = 2;
    setup.numvar = 168;
    setup.objective  = 'pogo_objective';
    setup.constraint = 'pogo_nonlcon';
    setup.auxdata=auxdata;
    
    adifuncs = adigatorGenFiles4Fmincon(setup);
    
    options=optimset(options,'GradObj','on','GradConstr','on','Hessian','user-supplied','HessFcn',adifuncs.hessian);
    Problem.options=options;
    
    Problem.objective = adifuncs.objgrd;%@(DecVar)pogo_objective(DecVar,nNodesF1,nNodesS,nNodesF2);
    Problem.nonlcon = adifuncs.congrd;%@(DecVar)pogo_nonlcon(DecVar,nNodesF1,nNodesS,nNodesF2,q0,dq0,pogo.parameters.lmin,pogo.parameters.lmax);
    %% SOLVE PROBLEM
