  ...
  elseif (length(CallingFile) > 16 && strcmp(CallingFile(1:16),'adigatortempfunc')) ||...
    ...
    if ADIGATOR.EMPTYFLAG
      ...
      if ~ADIGATOR.RUNFLAG
        ...
        cadaCancelDerivs(ADIGATOR.VARINFO.COUNT-1,numvars);
      end
      return
    end
...
elseif nargin == 2 && (nargout == 1 || nargout == 0)
  ...
  if ADIGATOR.FORINFO.FLAG
    ...
    if ADIGATOR.FORINFO.FLAG && ADIGATOR.RUNFLAG == 2
      if ADIGATOR.EMPTYFLAG && ~ADIGATOR.PRINT.FLAG
...
elseif nargout == 2
  ...
  if ~IndFlag
    y.func.value = outSize(1);
    y1.func.value = outSize(2);
    if isinf(outSize(1))
